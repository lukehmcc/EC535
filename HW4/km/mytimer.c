 ...
 */
struct file_operations mytimer_fops = {.write = mytimer_write,
                                       .read = mytimer_read,
                                       .open = mytimer_open,
...
static ssize_t mytimer_read(struct file *filep, char __user *buffer, size_t len,
  ...
  for (i = 0; i < MAX_TIMERS; ++i) {
    if (my_timer[i] && timer_pending(&my_timer[i]->timer)) {
      ...
      count += scnprintf(buf + count, sizeof(buf) - count, "%s %u\n",
                         my_timer[i]->msg, jiffies_to_msecs(left) / 1000U);
    }
  }
  ...
  if (req.command == 0) {
    ...
    for (i = 0; i < MAX_TIMERS; ++i) {
      ...
      if (timer_pending(&my_timer[i]->timer) &&
          strcmp(req.msg, my_timer[i]->msg) == 0) {
        mod_timer(&my_timer[i]->timer,
                  jiffies +
                      msecs_to_jiffies(1000 * req.seconds)); // For vmalloc
